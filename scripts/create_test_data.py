...
async def create_test_data(username: str, password: str, base_url: str = "http://localhost:8000"):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            elif "already exists" in analyst_response.text or analyst_response.status_code == 400:
                print("Analyst user already exists")
            else:
                print(f"Warning: Could not create analyst user: {analyst_response.status_code} - {analyst_response.text}")
            ...
            elif "already exists" in investigator_response.text or investigator_response.status_code == 400:
                print("Investigator user already exists")
            else:
                print(f"Warning: Could not create investigator user: {investigator_response.status_code} - {investigator_response.text}")
            
            # Get or create Personal client
            print("Getting clients list...")
            clients_response = await client.get(f"{base_url}/api/clients/", headers=headers)
            
            if clients_response.status_code != 200:
                print(f"ERROR: Failed to get clients: {clients_response.status_code} - {clients_response.text}")
                return
            
            try:
                clients = clients_response.json()
            except json.JSONDecodeError:
                print(f"ERROR: Invalid JSON response from clients API: {clients_response.text}")
                return
            ...
            if not personal_client:
                print("ERROR: Personal client not found. Please run init_db_auto.py first.")
                return
            ...
            if cases_response.status_code != 200:
                print(f"ERROR: Failed to get cases: {cases_response.status_code} - {cases_response.text}")
                return
            
            try:
                cases = cases_response.json()
            except json.JSONDecodeError:
                print(f"ERROR: Invalid JSON response from cases API: {cases_response.text}")
                return
            ...
            if not test_case:
                ...
                if case_response.status_code not in [200, 201]:
                    print(f"ERROR: Failed to create case: {case_response.status_code} - {case_response.text}")
                    return
            ...
            if template_response.status_code in [200, 201]:
                ...
                print(f"Warning: Could not apply Person template: {template_response.status_code} - {template_response.text}")
            ...
            if entities_response.status_code != 200:
                print(f"ERROR: Failed to get entities: {entities_response.status_code} - {entities_response.text}")
                return
            
            try:
                entities = entities_response.json()
            except json.JSONDecodeError:
                print(f"ERROR: Invalid JSON response from entities API: {entities_response.text}")
                return
            
            john_doe_exists = False
            for entity in entities:
                if (entity["entity_type"] == "person" and 
                    entity["data"].get("first_name") == "John" and 
                    entity["data"].get("last_name") == "Doe"):
            ...
            if not john_doe_exists:
                ...
                if entity_response.status_code not in [200, 201]:
                    print(f"ERROR: Failed to create entity: {entity_response.status_code} - {entity_response.text}")
                    return
            ...
            print(f"You can now log in and work with:")
            print(f"   - Case: Test Case 1 (#{test_case['case_number']})")
            print(f"   - Client: Personal")
            print(f"   - Entity: John Doe")
            print(f"   - Evidence folders: Person investigation template")
            print(f"")
            print(f"Available test users:")
            print(f"   - admin / admin (Admin role)")
            print(f"   - analyst / anapassword1 (Analyst role)")
            print(f"   - investigator / invpassword1 (Investigator role)")
