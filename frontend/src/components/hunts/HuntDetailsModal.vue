  ...
  <v-dialog v-model="dialog" max-width="800px" @keydown.esc="handleClose">
    <v-card>
      ...
      <v-card-text class="pa-4">
        <!-- Hunt Metadata -->
        <div class="mb-4">
          <div class="text-title-large mb-3">Hunt Information</div>

          <!-- Hunt Description -->
          <div v-if="hunt?.description" class="mb-4">
            <div class="text-body-large text-medium-emphasis">{{ hunt.description }}</div>
          </div>
          ...
          <v-table density="compact">
            ...
            <tbody>
              <tr v-for="(paramDef, paramName) in hunt.initial_parameters" :key="paramName">
                ...
                <td class="text-body-medium">{{ paramDef.description || 'No description' }}</td>
              </tr>
            </tbody>
          </v-table>
        </div>
        ...
        <div v-if="huntSteps && huntSteps.length > 0" class="mb-4">
          <div class="text-title-large mb-3">Hunt Workflow</div>
          <v-timeline density="compact" side="end">
            ...
            >
              <template #opposite>
                <span class="text-body-small">Step {{ index + 1 }}</span>
              </template>
              <div>
                <div class="text-body-large font-weight-medium">{{ step.display_name }}</div>
                <div class="text-body-medium text-medium-emphasis mb-1">{{ step.description }}</div>
                <div class="d-flex align-center">
                  <v-chip size="x-small" variant="outlined" class="me-2">
                    {{ step.plugin_name }}
                  </v-chip>
                  <span v-if="step.optional" class="text-body-small">Optional</span>
                </div>
              </div>
            </v-timeline-item>
          </v-timeline>
