...
class TestUsersAPI:
    ...
    def test_create_user_forbidden_non_admin(self, session: Session, test_user: User):
        ...
        try:
            with patch('app.services.user_service.UserService.create_user') as mock_create:
                ...
                mock_create.side_effect = HTTPException(status_code=403, detail="Permission denied")
    ...
    def test_create_user_duplicate_username(self, session: Session, test_admin: User):
        ...
        try:
            with patch('app.services.user_service.UserService.create_user') as mock_create:
                from fastapi import HTTPException
                mock_create.side_effect = HTTPException(status_code=400, detail="Username already registered")
    ...
    def test_get_users_success_admin(self, session: Session, test_admin: User, test_user: User):
    ...
    def test_get_users_forbidden_non_admin(self, session: Session, test_user: User):
        ...
        try:
            with patch('app.services.user_service.UserService.get_users') as mock_get:
                from fastapi import HTTPException
                mock_get.side_effect = HTTPException(status_code=403, detail="Permission denied")
    ...
    def test_update_user_success_admin(self, session: Session, test_admin: User, test_user: User):
    ...
    def test_update_user_not_found(self, session: Session, test_admin: User):
        ...
        try:
            with patch('app.services.user_service.UserService.update_user') as mock_update:
                from fastapi import HTTPException
                mock_update.side_effect = HTTPException(status_code=404, detail="User not found")
    ...
    def test_update_user_forbidden_other_user(self, session: Session, test_user: User, test_analyst: User):
    ...
    def test_change_password_wrong_current(self, session: Session, test_user: User):
        ...
        try:
            with patch('app.services.user_service.UserService.change_password') as mock_change:
                from fastapi import HTTPException
                mock_change.side_effect = HTTPException(status_code=400, detail="Current password is incorrect")
    ...
    def test_change_password_weak_password(self, session: Session, test_user: User):
        ...
        try:
            with patch('app.services.user_service.UserService.change_password') as mock_change:
                from fastapi import HTTPException
                mock_change.side_effect = HTTPException(status_code=400, detail="Password too weak")
    ...
    def test_admin_reset_password_success(self, session: Session, test_admin: User, test_user: User):
        ...
        try:
            with patch('app.services.user_service.UserService.admin_reset_password') as mock_reset:
                mock_reset.return_value = test_user
                
                response = client.put(f"/api/users/{test_user.id}/password", json=password_data)
                assert response.status_code == status.HTTP_200_OK
                data = response.json()
                assert data["id"] == test_user.id
        finally:
    ...
    def test_admin_reset_password_forbidden_non_admin(self, session: Session, test_user: User, test_analyst: User):
        ...
        try:
            with patch('app.services.user_service.UserService.admin_reset_password') as mock_reset:
                from fastapi import HTTPException
                mock_reset.side_effect = HTTPException(status_code=403, detail="Permission denied")
                
                response = client.put(f"/api/users/{test_analyst.id}/password", json=password_data)
                assert response.status_code == status.HTTP_403_FORBIDDEN
        finally:
            app.dependency_overrides.clear()

    def test_admin_reset_password_user_not_found(self, session: Session, test_admin: User):
        ...
        try:
            with patch('app.services.user_service.UserService.admin_reset_password') as mock_reset:
                from fastapi import HTTPException
                mock_reset.side_effect = HTTPException(status_code=404, detail="User not found")
    ...
    def test_users_api_unauthorized(self):
        ...
        endpoints = [
            ("POST", "/api/users/", {"username": "test", "email": "test@example.com", "password": "pass", "role": "Analyst", "is_active": True}),
            ("GET", "/api/users/"),
            ("PUT", "/api/users/1", {"username": "updated"}),
            ("PUT", "/api/users/me/password", {"current_password": "old", "new_password": "new"}),
    ...
    def test_users_api_error_format_consistency(self, session: Session, test_admin: User):
