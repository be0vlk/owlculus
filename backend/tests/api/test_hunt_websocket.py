...
class TestHuntWebSocket:
    ...
    async def test_websocket_progress_notifications(self):
        ...
        mock_websocket.send_json.assert_called_once_with(
            {"event_type": "progress", "progress": 0.5, "execution_id": execution_id}
        )
    ...
    async def test_websocket_completion_cleanup(self):
        ...
        # Verify the WebSocket received the message
        mock_websocket.send_json.assert_called_once_with(
            {"event_type": "complete", "execution_id": execution_id}
        )
    ...
    async def test_websocket_disconnected_client_handling(self):
        ...
        assert (
            execution_id not in websocket_manager.connections
            or mock_websocket
            not in websocket_manager.connections.get(execution_id, set())
        )
