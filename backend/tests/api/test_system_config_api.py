...
class TestSystemConfigAPI:
    ...
    def test_get_configuration_success_admin(self, session: Session, test_admin: User, test_system_config: SystemConfiguration):
        ...
        try:
            with patch('app.services.system_config_service.SystemConfigService.get_configuration') as mock_get:
    ...
    def test_get_configuration_creates_default(self, session: Session, test_admin: User):
    ...
    def test_get_configuration_forbidden_non_admin(self, session: Session, test_user: User):
    ...
    def test_get_configuration_forbidden_analyst(self, session: Session, test_analyst: User):
    ...
    def test_update_configuration_success_yymm_template(self, session: Session, test_admin: User, test_system_config: SystemConfiguration):
        ...
        try:
            with patch('app.services.system_config_service.SystemConfigService.update_configuration') as mock_update:
    ...
    def test_update_configuration_success_prefix_template(self, session: Session, test_admin: User, test_system_config: SystemConfiguration):
    ...
    def test_update_configuration_invalid_template(self, session: Session, test_admin: User):
    ...
    def test_update_configuration_missing_prefix_for_prefix_template(self, session: Session, test_admin: User):
    ...
    def test_update_configuration_invalid_prefix(self, session: Session, test_admin: User):
    ...
    def test_update_configuration_forbidden_non_admin(self, session: Session, test_user: User):
    ...
    def test_update_configuration_invalid_data_format(self, session: Session, test_admin: User):
    ...
    def test_preview_configuration_success_yymm(self, session: Session, test_admin: User):
        """Test successful configuration preview with YYMM-NN template"""
        app.dependency_overrides[get_current_user] = override_get_current_user_factory(test_admin)
        app.dependency_overrides[get_db] = override_get_db_factory(session)
        
        try:
            with patch('app.services.system_config_service.SystemConfigService.generate_example_case_number') as mock_generate:
                with patch('app.services.system_config_service.SystemConfigService.get_template_display_name') as mock_display:
    ...
    def test_preview_configuration_success_prefix(self, session: Session, test_admin: User):
        ...
        try:
            with patch('app.services.system_config_service.SystemConfigService.generate_example_case_number') as mock_generate:
                with patch('app.services.system_config_service.SystemConfigService.get_template_display_name') as mock_display:
                    ...
                    mock_display.return_value = "Prefix + Monthly Reset (PREFIX-YYMM-NN)"
                    
                    response = client.get("/api/admin/configuration/preview?template=PREFIX-YYMM-NN&prefix=CORP")
                    assert response.status_code == status.HTTP_200_OK
                    data = response.json()
                    assert data["template"] == "PREFIX-YYMM-NN"
                    assert data["prefix"] == "CORP"
                    assert data["example_case_number"] == "CORP-2506-01"
                    assert data["display_name"] == "Prefix + Monthly Reset (PREFIX-YYMM-NN)"
        finally:
            app.dependency_overrides.clear()

    def test_preview_configuration_invalid_template(self, session: Session, test_admin: User):
        """Test configuration preview with invalid template"""
        app.dependency_overrides[get_current_user] = override_get_current_user_factory(test_admin)
        app.dependency_overrides[get_db] = override_get_db_factory(session)
        
        try:
            response = client.get("/api/admin/configuration/preview?template=INVALID-TEMPLATE")
    ...
    def test_preview_configuration_missing_template(self, session: Session, test_admin: User):
    ...
    def test_preview_configuration_forbidden_non_admin(self, session: Session, test_user: User):
    ...
    def test_configuration_api_special_characters_in_prefix(self, session: Session, test_admin: User):
    ...
    def test_configuration_api_prefix_too_long(self, session: Session, test_admin: User):
    ...
    def test_configuration_api_case_insensitive_prefix(self, session: Session, test_admin: User, test_system_config: SystemConfiguration):
    ...
    def test_configuration_api_null_values(self, session: Session, test_admin: User, test_system_config: SystemConfiguration):
    ...
    def test_configuration_api_response_format_consistency(self, session: Session, test_admin: User, test_system_config: SystemConfiguration):
        ...
        try:
            with patch('app.services.system_config_service.SystemConfigService.get_configuration') as mock_get:
                ...
                required_fields = ["id", "case_number_template", "case_number_prefix", "created_at", "updated_at"]
    ...
    def test_preview_configuration_empty_prefix(self, session: Session, test_admin: User):
        ...
        try:
            with patch('app.services.system_config_service.SystemConfigService.generate_example_case_number') as mock_generate:
                with patch('app.services.system_config_service.SystemConfigService.get_template_display_name') as mock_display:
                    ...
                    response = client.get("/api/admin/configuration/preview?template=YYMM-NN&prefix=")
                    assert response.status_code == status.HTTP_200_OK
                    data = response.json()
                    assert data["prefix"] == ""
        finally:
