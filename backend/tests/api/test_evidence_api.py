...
class TestEvidenceAPI:
    ...
    def test_get_case_evidence_success(self, session: Session, test_admin: User, test_case: Case, test_evidence: Evidence):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.get_case_evidence') as mock_get:
    ...
    def test_get_case_evidence_with_pagination(self, session: Session, test_admin: User, test_case: Case):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.get_case_evidence') as mock_get:
                ...
                response = client.get(f"/api/evidence/case/{test_case.id}?skip=10&limit=5")
                assert response.status_code == status.HTTP_200_OK
        finally:
            app.dependency_overrides.clear()

    def test_get_case_evidence_forbidden_non_assigned(self, session: Session, test_user: User, test_case: Case):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.get_case_evidence') as mock_get:
                ...
                mock_get.side_effect = HTTPException(status_code=403, detail="Not authorized")
    ...
    def test_get_evidence_success(self, session: Session, test_admin: User, test_evidence: Evidence):
        """Test successful evidence retrieval"""
        app.dependency_overrides[get_current_active_user] = override_get_current_user_factory(test_admin)
        app.dependency_overrides[get_db] = override_get_db_factory(session)
        
        try:
            with patch('app.services.evidence_service.EvidenceService.get_evidence') as mock_get:
                mock_get.return_value = test_evidence
    ...
    def test_get_evidence_not_found(self, session: Session, test_admin: User):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.get_evidence') as mock_get:
                from fastapi import HTTPException
                mock_get.side_effect = HTTPException(status_code=404, detail="Evidence not found")
    ...
    def test_download_evidence_success(self, session: Session, test_admin: User, test_evidence: Evidence):
        """Test successful evidence download"""
        app.dependency_overrides[get_current_active_user] = override_get_current_user_factory(test_admin)
        app.dependency_overrides[get_db] = override_get_db_factory(session)
        
        try:
            with patch('app.services.evidence_service.EvidenceService.download_evidence') as mock_download:
                # Create a simple response instead of FileResponse which requires real file
                from fastapi.responses import Response
                mock_response = Response(content=b"test file content", media_type="application/octet-stream")
    ...
    def test_download_evidence_not_found(self, session: Session, test_admin: User):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.download_evidence') as mock_download:
                from fastapi import HTTPException
                mock_download.side_effect = HTTPException(status_code=404, detail="Evidence not found")
    ...
    def test_update_evidence_success(self, session: Session, test_admin: User, test_evidence: Evidence):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.update_evidence') as mock_update:
                ...
                response = client.put(f"/api/evidence/{test_evidence.id}", json=update_data)
                assert response.status_code == status.HTTP_200_OK
                data = response.json()
    ...
    def test_update_evidence_not_found(self, session: Session, test_admin: User):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.update_evidence') as mock_update:
                from fastapi import HTTPException
                mock_update.side_effect = HTTPException(status_code=404, detail="Evidence not found")
    ...
    def test_update_evidence_forbidden_analyst(self, session: Session, test_analyst: User, test_evidence: Evidence):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.update_evidence') as mock_update:
                from fastapi import HTTPException
                mock_update.side_effect = HTTPException(status_code=403, detail="Permission denied")
    ...
    def test_delete_evidence_success(self, session: Session, test_admin: User, test_evidence: Evidence):
        """Test successful evidence deletion"""
        app.dependency_overrides[get_current_active_user] = override_get_current_user_factory(test_admin)
        app.dependency_overrides[get_db] = override_get_db_factory(session)
        
        try:
            with patch('app.services.evidence_service.EvidenceService.delete_evidence') as mock_delete:
    ...
    def test_delete_evidence_not_found(self, session: Session, test_admin: User):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.delete_evidence') as mock_delete:
                from fastapi import HTTPException
                mock_delete.side_effect = HTTPException(status_code=404, detail="Evidence not found")
    ...
    def test_delete_evidence_forbidden_analyst(self, session: Session, test_analyst: User, test_evidence: Evidence):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.delete_evidence') as mock_delete:
                from fastapi import HTTPException
                mock_delete.side_effect = HTTPException(status_code=403, detail="Permission denied")
    ...
    def test_create_folder_success(self, session: Session, test_admin: User, test_case: Case):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.create_folder') as mock_create:
    ...
    def test_get_folder_tree_success(self, session: Session, test_admin: User, test_case: Case, test_folder: Evidence):
        """Test successful folder tree retrieval"""
        app.dependency_overrides[get_current_active_user] = override_get_current_user_factory(test_admin)
        app.dependency_overrides[get_db] = override_get_db_factory(session)
        
        try:
            with patch('app.services.evidence_service.EvidenceService.get_folder_tree') as mock_get:
    ...
    def test_update_folder_success(self, session: Session, test_admin: User, test_folder: Evidence):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.update_folder') as mock_update:
                ...
                response = client.put(f"/api/evidence/folders/{test_folder.id}", json=update_data)
                assert response.status_code == status.HTTP_200_OK
                data = response.json()
    ...
    def test_delete_folder_success(self, session: Session, test_admin: User, test_folder: Evidence):
        """Test successful folder deletion"""
        app.dependency_overrides[get_current_active_user] = override_get_current_user_factory(test_admin)
        app.dependency_overrides[get_db] = override_get_db_factory(session)
        
        try:
            with patch('app.services.evidence_service.EvidenceService.delete_folder') as mock_delete:
    ...
    def test_evidence_api_pagination_edge_cases(self, session: Session, test_admin: User, test_case: Case):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.get_case_evidence') as mock_get:
                ...
                response = client.get(f"/api/evidence/case/{test_case.id}?skip=999999&limit=999999")
                assert response.status_code == status.HTTP_200_OK
                
                # Test with zero limit
                response = client.get(f"/api/evidence/case/{test_case.id}?skip=0&limit=0")
                assert response.status_code == status.HTTP_200_OK
    ...
    def test_evidence_api_invalid_case_id(self, session: Session, test_admin: User):
        ...
        try:
            with patch('app.services.evidence_service.EvidenceService.get_case_evidence') as mock_get:
                from fastapi import HTTPException
                mock_get.side_effect = HTTPException(status_code=404, detail="Case not found")
    ...
    def test_evidence_api_error_format_consistency(self, session: Session, test_admin: User):
    ...
    def test_evidence_api_response_time(self, session: Session, test_admin: User, test_case: Case):
