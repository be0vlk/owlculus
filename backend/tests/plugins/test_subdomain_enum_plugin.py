...
class TestSubdomainEnumPlugin:
        ...
        with patch("aiohttp.ClientSession") as mock_session:
            ...
            with patch("dns.asyncresolver.Resolver") as mock_resolver_class:
                ...
                status_messages = [
                    r
                    for r in results
                    if r.get("data", {}).get("phase") in ["discovery", "resolution"]
                ]
                assert len(status_messages) > 0

                # Check for subdomain results
                subdomain_results = [
                    r for r in results if r.get("data", {}).get("subdomain")
                ]
    ...
    def test_format_evidence_content(self, plugin):
        ...
        params = {"domain": "example.com", "execution_time": "2024-01-01 12:00:00"}
