...
class TestCreateFolder:
    ...
    def test_create_folder_invalid_path_after_normalization(self, temp_upload_dir, case_id):
...
class TestSaveUploadFile:
    ...
    @pytest.mark.asyncio
    async def test_save_upload_file_valid_case(self, temp_upload_dir, case_id, sample_upload_file):
        """Test saving upload file with valid case ID."""
        with patch('app.core.file_storage.validate_file_security') as mock_validate, \
             patch('app.core.file_storage.secure_filename_with_path') as mock_secure:
    ...
    @pytest.mark.asyncio
    async def test_save_upload_file_with_folder_path(self, temp_upload_dir, case_id, sample_upload_file):
        ...
        with patch('app.core.file_storage.validate_file_security') as mock_validate, \
            ...
            result = await file_storage.save_upload_file(sample_upload_file, case_id, folder_path)
    ...
    @pytest.mark.asyncio
    async def test_save_upload_file_none_case_id(self, temp_upload_dir, sample_upload_file):
    ...
    @pytest.mark.asyncio
    async def test_save_upload_file_invalid_case_id(self, temp_upload_dir, sample_upload_file):
        """Test that invalid case ID raises HTTPException."""
        with pytest.raises(HTTPException) as exc_info:
    ...
    @pytest.mark.asyncio
    async def test_save_upload_file_validation_fails(self, temp_upload_dir, case_id, sample_upload_file):
        """Test that file validation failure is propagated."""
        with patch('app.core.file_storage.validate_file_security') as mock_validate:
            mock_validate.side_effect = HTTPException(status_code=400, detail="File not allowed")
    ...
    @pytest.mark.asyncio
    async def test_save_upload_file_generic_exception_handling(self, temp_upload_dir, case_id, sample_upload_file):
...
class TestSecurityIntegration:
    ...
    def test_normalize_path_security_comprehensive(self):
        ...
        for dangerous_input in dangerous_inputs:
            ...
            assert not result or not any(dangerous in result for dangerous in ["../", "..\\", "/.."])
