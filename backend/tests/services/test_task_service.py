...
class TestTaskTemplateOperations:
    ...
    async def test_get_templates_all(
        ...
        task_template: models.TaskTemplate,
        inactive_template: models.TaskTemplate,
        test_admin: models.User,
    ):
...
class TestTaskFiltering:
    ...
    async def test_get_tasks_by_case(
        ...
        tasks = await task_service.get_tasks(
            current_user=test_admin, case_id=test_case.id
        )
        assert len(tasks) == 4
    ...
    async def test_get_tasks_by_status(
        self, task_service: TaskService, multiple_tasks: list, test_admin: models.User
    ):
        """Test getting tasks filtered by status"""
        # Get completed tasks
        tasks = await task_service.get_tasks(
            current_user=test_admin, status=TaskStatus.COMPLETED.value
        )
        assert len(tasks) == 1
        ...
        tasks = await task_service.get_tasks(
            current_user=test_admin, status=TaskStatus.IN_PROGRESS.value
        )
        assert len(tasks) == 1
    ...
    async def test_get_tasks_by_priority(
        ...
        tasks = await task_service.get_tasks(
            current_user=test_admin, priority=TaskPriority.HIGH.value
        )
        assert len(tasks) == 2
        for task in tasks:
            assert task.priority == TaskPriority.HIGH.value
    ...
    async def test_get_tasks_pagination(
        ...
        # Get first 2 tasks
        tasks = await task_service.get_tasks(current_user=test_admin, skip=0, limit=2)
        assert len(tasks) == 2

        # Get next 2 tasks
        tasks = await task_service.get_tasks(current_user=test_admin, skip=2, limit=2)
        assert len(tasks) == 2
...
class TestTaskAssignment:
    ...
    async def test_assign_task(
        ...
        case_link = models.CaseUserLink(
            case_id=sample_task.case_id, user_id=test_investigator.id
        )
        task_service.db.add(case_link)
        task_service.db.commit()
    ...
    async def test_bulk_assign_tasks(
        ...
        case_link = models.CaseUserLink(
            case_id=multiple_tasks[0].case_id, user_id=test_investigator.id
        )
        task_service.db.add(case_link)
        task_service.db.commit()
...
class TestTaskStatusOperations:
    ...
    async def test_update_status_from_completed(
        self, task_service: TaskService, test_case: models.Case, test_admin: models.User
    ):
...
class TestTaskAccessControl:
    ...
    async def test_analyst_read_access(
        ...
        case_link = models.CaseUserLink(
            case_id=sample_task.case_id, user_id=test_analyst.id
        )
        task_service.db.add(case_link)
        task_service.db.commit()
