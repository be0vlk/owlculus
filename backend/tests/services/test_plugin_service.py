...
class TestPluginService:
    ...
    async def test_list_plugins_analyst_permission(
        ...
        with patch('app.core.dependencies.no_analyst') as mock_decorator:
            ...
            def side_effect(*args, **kwargs):
                if 'current_user' in kwargs and kwargs['current_user'].role == 'analyst':
    ...
    async def test_execute_plugin_success(
        ...
        with patch('test_plugin_service.MockPlugin.execute_with_evidence_collection') as mock_execute:
    ...
    async def test_execute_plugin_no_params(
        ...
        with patch.object(MockPlugin, 'execute_with_evidence_collection') as mock_execute:
            async def mock_generator():
                yield {"type": "data", "data": {"test": "result"}}
    ...
    def test_load_plugins_directory_structure(self, mock_listdir, mock_dirname, session: Session):
    ...
    def test_load_plugins_import_error_handling(self, mock_import, mock_listdir, session: Session):
    ...
    def test_load_plugins_filters_correctly(self, mock_getmembers, mock_import, mock_listdir, session: Session):
        ...
        class AnotherMockPlugin(BasePlugin):
            def __init__(self):
                super().__init__()
            def parse_output(self, line): return None
    ...
    def test_get_plugin_returns_new_instance(self, plugin_service_instance: PluginService):
