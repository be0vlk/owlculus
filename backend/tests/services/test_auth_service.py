...
class TestAuthenticationService:
    ...
    async def test_authenticate_invalid_password(
        self, auth_service, mock_user_repository, mock_password_hasher, test_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_authenticate_system_error(self, auth_service, mock_user_repository):
...
class TestWebSocketTokenService:
    ...
    def ws_token_service(
        self, mock_execution_repository, mock_case_access_checker, mock_token_generator
    ):
    ...
    async def test_create_token_execution_not_found(
        self, ws_token_service, mock_execution_repository, test_user
    ):
...
class TestAuthServiceFacade:
    ...
    async def test_create_websocket_token_facade(self, auth_service):
        ...
        mock_ws_result = WebSocketToken(
            token="ws_token", execution_id=100, expires_in=30
        )
        ...
        assert result == {"token": "ws_token", "execution_id": 100, "expires_in": 30}
...
class TestDatabaseImplementations:
    ...
    def test_database_case_access_checker_has_access(self, mock_db):
        ...
        mock_check.assert_called_once_with(mock_user, 10)
...
class TestUtilityClasses:
    ...
    def test_jwt_token_generator_access_token(self):
        ...
        mock_create.assert_called_once_with(data={"sub": "user"}, expires_delta=expires)
    ...
    def test_jwt_token_generator_ephemeral_token(self):
        ...
        with patch(
            "app.services.auth_service.security.ephemeral_token_manager.create_token"
        ) as mock_create:
