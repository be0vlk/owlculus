...
async def check_entity_duplicates(
    ...
    if entity_type == "company":
        ...
        if domains or ip_addresses or subdomains:
            conditions = []
            if domains:
                conditions.append(or_(*[models.Entity.data["domains"].as_array().any(d.lower()) for d in domains]))
            if ip_addresses:
                conditions.append(or_(*[models.Entity.data["ip_addresses"].as_array().any(ip.lower()) for ip in ip_addresses]))
            if subdomains:
                conditions.append(or_(*[models.Entity.data["subdomains"].as_array().any(s.lower()) for s in subdomains]))
