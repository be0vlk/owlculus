"""
Authentication and WebSocket authorization service for Owlculus OSINT platform.

This module provides comprehensive authentication services including user login,
...
class AuthenticationService:
    ...
    async def _verify_user_credentials(
        self, username: str, password: str, logger
    ) -> User:
    ...
    def _get_auth_logger(self, username: str, action: str, event_type: str):
        return get_security_logger(
            username=username, action=action, event_type=event_type
        )
    ...
    def _log_failed_auth(
        self, logger, reason: str, user_id: Optional[int] = None
    ) -> None:
        log_data = {"event_type": "login_failed", "failure_reason": reason}
        if user_id:
...
class WebSocketTokenService:
    ...
    def _get_and_validate_execution(self, execution_id: int, logger) -> HuntExecution:
    ...
    def _validate_case_access(
        self, execution: HuntExecution, user: User, logger
    ) -> None:
        if not self.case_access_checker.has_access(execution.case_id, user):
            self._log_token_failed(logger, "access_denied", case_id=execution.case_id)
    ...
    def _log_token_failed(self, logger, reason: str, **kwargs) -> None:
        log_data = {"event_type": "ws_token_failed", "failure_reason": reason, **kwargs}
