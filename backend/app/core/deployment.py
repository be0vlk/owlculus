        ...
        if not value or value.lower() in PLACEHOLDERS:
            ...
            if name == "RUNTIME_POSTGRES_PASSWORD":
                guidance = (
                    ...
                    "\nTo upgrade an install without a runtime login:"
                    "\n1. Run: openssl rand -hex 32"
                    "\n2. Edit .env in the repository root. Add "
                    "RUNTIME_POSTGRES_USER=owlculus_runtime (different from POSTGRES_USER) "
                    "and RUNTIME_POSTGRES_PASSWORD=<paste the generated output>. "
                    "If that login already exists, restore its existing password instead."
                    "\n3. Save .env and rerun: make setup"
                    "\nSetup creates the login on startup. "
                    "Existing installs must preserve SECRET_KEY and POSTGRES_PASSWORD "
                    "and keep their existing POSTGRES_USER and POSTGRES_DB."
                )
            raise ValueError(
