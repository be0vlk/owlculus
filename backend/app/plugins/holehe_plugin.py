...
class HolehePlugin(BasePlugin):
    ...
    async def _get_holehe_modules(self):
        ...
        try:
            ...
            for importer, modname, ispkg in pkgutil.walk_packages(holehe.__path__, holehe.__name__ + '.'):
        ...
        try:
            ...
            if out:
                ...
                return {
                    ...
                    "partial_info": result.get("emailrecovery") or result.get("phoneNumber"),
    ...
    async def run(
        ...
        if not email:
            yield {"type": "error", "data": {"message": "Email address cannot be empty"}}
            return
