...
function removeScripts(element) {
  ...
  const onEventAttrs = Array.from(element.querySelectorAll('*'))
    .filter(el => {
...
function removeSensitiveData(element) {
  ...
  const csrfTokens = element.querySelectorAll('[name*="csrf"], [name*="token"], [data-csrf]');
  csrfTokens.forEach(el => {
    if (el.value) el.value = '[REDACTED]';
    if (el.hasAttribute('data-csrf')) el.setAttribute('data-csrf', '[REDACTED]');
  });
}